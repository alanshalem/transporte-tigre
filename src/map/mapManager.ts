...
const CAPITAN: LatLng[] = [
  ...
  [-34.2947, -58.5724],
  [-34.2933, -58.5705],
  [-34.2900, -58.5665],   // Paraná de las Palmas junction (N)
];
...
const PARANA_SE: LatLng[] = [
  ...
  [-34.2846, -58.5773],
  [-34.2885, -58.5694],
  [-34.2900, -58.5665],   // Capitán junction
];
...
function head(s: LatLng[], n: number): LatLng[] { return s.slice(0, n); }
...
defaultRoutes['451-troncal'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, VIA_CAPITAN,
  PARANA_CAP_TO_PAY.slice(1), PAYCARABI.slice(1)
);

// Ramal 1: Capitán → Paraná → Paycarabí parcial → Paraná Mini
defaultRoutes['451-ramal1'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, VIA_CAPITAN,
  PARANA_CAP_TO_PAY.slice(1), head(PAYCARABI, 25).slice(1)
);
...
defaultRoutes['452-troncal'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, VIA_CAPITAN,
  PARANA_CAP_TO_PAY.slice(1), head(PAYCARABI, 30).slice(1)
);

// Fraccionado: Capitán → Paraná → Paycarabí corto
defaultRoutes['452-fraccionado'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, VIA_CAPITAN,
  PARANA_CAP_TO_PAY.slice(1), head(PAYCARABI, 15).slice(1)
);

// Ramal 1: Sarmiento parcial → Río San Antonio
defaultRoutes['452-ramal1'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, head(SARMIENTO, 20).slice(1)
);

// Ramal 2: Sarmiento parcial → San Antonio → Dorado
defaultRoutes['452-ramal2'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, head(SARMIENTO, 24).slice(1)
);

// Ramal 3: Sarmiento → Capitán parcial (Rama Negra / Toro)
defaultRoutes['452-ramal3'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, head(CAPITAN, 20).slice(1)
);

// Ramal 4: Sarmiento parcial (Arroyo Espera)
defaultRoutes['452-ramal4'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, head(SARMIENTO, 12).slice(1)
);

// Ramal 5: Luján E → Sarmiento corto (Arroyo Abra Vieja)
defaultRoutes['452-ramal5'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, head(SARMIENTO, 8).slice(1)
);
...
defaultRoutes['452-fraccionado10'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, head(SARMIENTO, 16).slice(1)
);
...
defaultRoutes['454-troncal'] = buildRoute(
  FROM_STATION, VIA_LUJAN_E, VIA_SARMIENTO, VIA_CAPITAN,
  PARANA_CAP_TO_PAY.slice(1), head(PAYCARABI, 20).slice(1)
);
...
defaultRoutes['454-desdoblamiento'] = buildRoute(
  FROM_STATION, VIA_LUJAN_W, VIA_LUJAN_W2, VIA_LUJAN_W3,
  VIA_CANAL_ARIAS_UP, PARANA_ARIAS_TO_PAY.slice(1), head(PAYCARABI, 15).slice(1)
);
...
defaultRoutes['455-ramal1'] = buildRoute(
  ...
  [[-34.2260, -58.7980],         // into Arroyo Fermín channel (OSM way 313713812)
   [-34.2243, -58.8057],
   [-34.2220, -58.8150],
   [-34.2197, -58.8268]]
);
...
export class MapManager {
  ...
  showBoats(boats: { lineId: number; lineColor: string; position: LatLng; serviceType: string; direction: string; departureTime: string; destination: string }[]): void {
    this.clearBoats();
    if (!this.map) return;
    ...
    for (const boat of boats) {
      ...
      const marker = L.marker(boat.position, { icon, interactive: true, zIndexOffset: 1000 })
        .addTo(this.map);
